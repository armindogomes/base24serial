...
public class Base24SerialConverter {
    ...
    public static string Convert(string hexValue) {
        ...
        if (hexValue.Count() != HEX_STRING_LENGTH) throw new ArgumentOutOfRangeException();

        byte[] byteArray;
        try {
            byteArray = HexStringToByteArray(hexValue);
        }
    ...
    private static byte[] HexStringToByteArray(string hexValue) {
        try {
            return Enumerable.Range(0, hexValue.Length / 2).Select(x => System.Convert.ToByte(hexValue.Substring(x * 2, 2), 16)).ToArray();
        }
        catch (ArgumentOutOfRangeException) {
            throw;
        }
